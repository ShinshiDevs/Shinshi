...
class Cache(CacheImpl):
    ...
    def __init__(
        ...
        self.null_messages: LFUCache[int, None] = LFUCache(MESSAGE_NULL_CACHE_SIZE)
        self.null_channels: LFUCache[int, None] = LFUCache(CHANNEL_NULL_CACHE_SIZE)
        self.members: LFUCache[tuple[int, ...], Member | None] = LFUCache(
            MEMBER_CACHE_SIZE
        )

    def clear_safe(self) -> None:
        self.null_messages.clear()
        self.null_channels.clear()
        self.members.clear()
        self.clear_messages()
