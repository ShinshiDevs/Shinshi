...
async def kick_hook(context: Context) -> HookResult:
    ...
    if context.bot.get_me().id == target.id:
        await context.create_warning_response(
            context.locale.get("commands.kick.errors.target_is_bot")
        )
        return HookResult(stop=True)

    if moderator.id == target.id:
        await context.create_warning_response(
            context.locale.get("commands.kick.errors.target_is_moderator")
        )
        return HookResult(stop=True)

    if bot.get_top_role().position <= target.get_top_role().position:
        await context.create_warning_response(
            context.locale.get("commands.kick.errors.target_too_high_for_bot")
        )
        return HookResult(stop=True)

    if (
        moderator.id != context.guild.owner_id
        and moderator.get_top_role().position <= target.get_top_role().position
    ):
        await context.create_warning_response(
            context.locale.get("commands.kick.errors.target_too_high_for_moderator")
        )
        return HookResult(stop=True)
